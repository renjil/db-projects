...
- 1 Lakebase Autoscaling **project** with project-level default endpoint settings + 30-day PITR
...
- 1 **group-backed analyst role** (read-only)
...
lakebase_e2e/
├── README.md
├── versions.tf            # provider pins + remote backend placeholder
├── providers.tf
├── variables.tf
├── locals.tf
...
├── roles_and_database.tf  # service principal + group roles, app database
...
1. Terraform >= 1.5
2. An OAuth M2M service principal in the target Databricks workspace with permission to manage Lakebase resources
3. Credentials supplied via either Terraform variables (see `terraform.tfvars.example`) or environment variables:
   ```bash
   export DATABRICKS_HOST="https://<workspace>.cloud.databricks.com"
   export DATABRICKS_CLIENT_ID="<sp-application-id>"
   export DATABRICKS_CLIENT_SECRET="<sp-secret>"
   ```
4. A remote state backend (S3/Azure Blob/GCS) - **required**, Autoscaling has no drift detection so local state is unsafe. The example pins an `s3` backend with placeholder values in `versions.tf` - replace before `terraform init`.
